...
import {deleteCartPizzas, deletePizzaFromCart} from "../redux/actions/cart";

const Cart = () => {
    const dispatch = useDispatch();
    const deleteAllPizzas = () => {
        const mess = window.confirm('Вы уверены?');
        if (mess){
            dispatch(deleteCartPizzas());
        }
    }
    const deletePizza = (obj) => {
        dispatch(deletePizzaFromCart(obj));
    }
    const cart = useSelector(({cart}) => cart);
    console.log(cart);
    const ttt = Object.values(cart.pizzas).map(item => {
        return item.map(obj=>{
            return <CartItem {...obj} deletePizza={deletePizza}/>
        })
    })
                ...
                <div className="container container--cart">
                    <div className="cart">
                        ...
                        <div className="content__items">
                            {
                                ttt
                                // Object.keys(cart.pizzas).map(item => {
                                //     return item.map(obj=>{
                                //         return <CartItem {...obj}/>
                                //     })
                                // })
                        ...
                        <div className="cart__bottom">
                            <div className="cart__bottom-details">
                                <span> Всего пицц: <b>{cart.totalPizzas} шт.</b> </span>
